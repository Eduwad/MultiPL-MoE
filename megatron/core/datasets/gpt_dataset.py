...
class GPTDataset(MegatronDataset):
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, torch.Tensor]:
        ...
        indices = numpy.argwhere(text == 151643)[0][0]
        # indices = numpy.argwhere(text == 32013)[0][0]
        text_1 = text[:indices]

        # task_choice = random.choices(['ntp', 'fim'], weights=[0.7, 0.3])[0]
        # task_choice = random.choices(['ntp', 'fim'], weights=[1.0, 0.0])[0]
        task_choice = 'ntp'
        if task_choice == 'ntp':
            if self.config.add_extra_token_to_sequence:
                tokens = text_1[:-1].contiguous()
                labels = text_1[1:].contiguous()
            else:
                tokens = text_1
                labels = torch.roll(text_1, shifts=-1, dims=0)
                labels[-1] = self._pad_token_id
        else:
            # 根据规则决定是从中间还是开头进行掩码操作
            mask_position_choice = random.choices(['middle', 'start'], weights=[0.7, 0.3])[0]
            # 中间填充任务的数据处理
            seq_length = len(text_1)
            if seq_length <= 3:
                tokens = text_1[:-1].contiguous()
                labels = text_1[1:].contiguous()

                mask_position_choice = -1
            elif seq_length > max_seq_length - 4:  # 如果序列长度过长 需要进行截断  因为会添加特殊的token
                input_ids = text_1[:max_seq_length - 4].contiguous().tolist()  # 以max_seq_length - 4进行截断处理
                seq_length = len(input_ids)
            else:
                input_ids = text_1.tolist()

            
            if mask_position_choice == 'middle':
                # 在文本中间随机选择两个数字a和b，a和b之间差值最小为1，将数据划分为三段
                a = random.randint(0, seq_length - 2)
                b = random.randint(a + 1, seq_length - 1)
                prefix = input_ids[:a]
                middle = input_ids[a:b]
                suffix = input_ids[b:]
                tokens = (
                    [151646] + prefix +
                    [151648] + suffix +
                ...
                labels = [-100] * (len(prefix) + len(suffix) + 3) + middle + [self._pad_token_id] 
            elif mask_position_choice == 'start':
                # 在文本开头进行掩码，在(0，seq_length - 3)中随机选取2个数字a和b，a和b之间差值最小为1，将数据划分为三段
                a = random.randint(0, seq_length - 3)
                b = random.randint(a + 1, seq_length - 2)
                middle = input_ids[a:b]
                suffix = input_ids[b:]
                prefix = input_ids[:a]
                tokens = (
                ...
                labels = [-100] * (len(middle) + len(suffix) + 3) + prefix + [self._pad_token_id]

        tokens = torch.LongTensor(tokens)
        labels = torch.LongTensor(labels)
        assert len(tokens) == len(labels)
        tokens_padded = torch.full((1024,), self._pad_token_id, dtype=torch.long)
        labels_padded = torch.full((1024,), self._pad_token_id, dtype=torch.long)
        ...
        if torch.all(labels[ :3] == -100):
            loss_mask[:] = 0.0
        # loss_mask[labels == -100] = 0.0

        # For padded sequences, ensure the embedding layer can map the token ID
        tokens[tokens == self._pad_token_id] = 0.0
        labels[labels == self._pad_token_id] = 0.0
