...
class SequentialMLP(MegatronModule):
    ...
    def forward(self, permuted_local_hidden_states, tokens_per_expert, X_in, scores_segment, indices_segment):
        ...
        for expert_num, expert in enumerate(self.local_experts):
            if expert_num < self.config.num_moe_experts//2:
                ...
                out = expert(X_in[expert_num - self.config.num_moe_experts//2])[0]
                X_expert.append(out)
        expert_out = torch.stack(X_expert, dim=0).to(indices_segment.device)
        y_seg= torch.einsum('ijl,ij,ijd->ld', indices_segment, scores_segment, expert_out)

        return output_local, output_bias_local, y_seg, None
