...
def topk_softmax_with_capacity(
    ...
    if use_pre_softmax:
        ...
        probs, top_indices = torch.topk(scores[:,:-1], k=topk//2-1, dim=1) # batch*seq_length,topk-1

        shared_expert_weight = scores[:,-1].reshape(-1,1)

        router_output = torch.cat([probs, shared_expert_weight], dim=-1).view(-1, topk//2)
        router_output = router_output * (1 / router_output.sum(dim=1, keepdim=True).expand_as(router_output))

        new_part = torch.full((num_tokens,1), num_experts - 1, dtype=top_indices.dtype, device=top_indices.device)
        new_indices = torch.cat([top_indices, new_part], dim=1)
    else:
        # Post softmax
        if topk == 1:
    ...
    if capacity_factor is None:
        ...
        tokens_per_expert = torch.bincount(new_indices.view(-1), minlength=num_experts)
        return router_output, new_indices, tokens_per_expert,scores
    else:
        # TopK with capacity
