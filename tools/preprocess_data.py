...
def split_text(tokens):
    if len(tokens) > 1024:
        ...
        return [first_part_tokens] + split_text(second_part_tokens)
    else:
        return [tokens]
...
class Encoder(object):
    ...
    def encode(self, json_line):
        ...
        sentence_lens = []
        re_data = []
        # print(f"json num:{len(sentences)}")
        for sentence in sentences:
            sentence_ids = Encoder.tokenizer.tokenize(sentence)
            if len(sentence_ids) > 1024:
                # print(f"sentence:{len(sentence_ids)}")
                split_parts = split_text(sentence_ids)
                tokens = split_parts
                # print(f"sentence_num:{len(tokens)}")
            else:
                tokens = [sentence_ids]
            for i in tokens:
                doc_ids.extend(i)
                sentence_lens.append(len(i))
                if len(doc_ids) > 0 and self.args.append_eod:
                    doc_ids.append(Encoder.tokenizer.eod)
                    sentence_lens[-1] += 1
                ids[key] = doc_ids
                lens[key] = sentence_lens
                re_data.append([ids, lens, len(json_line)])
        return re_data
...
class Partition(object):
    ...
    def process_json_file(self, file_name):
        ...
        print(type(encoded_docs))
        #传回的可能是字典，也可能是list[dict,dict,dict]
        encoded_docs_1 = [item for sublist in encoded_docs for item in sublist]
        print(type(encoded_docs_1))
        ...
        for i, (doc, sentence_lens, bytes_processed) in enumerate(encoded_docs_1, start=1):
            total_bytes_processed += bytes_processed
            for key in doc.keys():
...
def get_args():
    ...
    group.add_argument('--json-keys', nargs='+', default=['content'],
