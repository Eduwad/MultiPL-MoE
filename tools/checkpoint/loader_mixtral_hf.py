...
def load_args_from_checkpoint(args):
    ...
    args.untie_embeddings_and_output_weights = not mixtral_config.tie_word_embeddings
    ...
    args.moe_ffn_hidden_size = mixtral_config.intermediate_size
...
def set_attn_state(args, layer, hf_layer):
    ...
    if args.add_qkv_bias:
        print
        q_proj_bias = hf_attn.q_proj.bias.view(num_query_groups, -1) # 2 x 1024
        k_proj_bias = hf_attn.k_proj.bias.view(num_query_groups, -1) # 2 x 128
        v_proj_bias = hf_attn.v_proj.bias.view(num_query_groups, -1)
        qkv_bias = torch.cat([q_proj_bias, k_proj_bias, v_proj_bias], dim=1).view(-1).contiguous()
        attn.linear_qkv.bias.data.copy_(qkv_bias)
...
def load_checkpoint_to_model(args):
    ...
    # hf_model_qw = torch.load("/root/work/filestorage/xinglehao/moe/Megatron-LM/models/qwen1.5-1.8B")
